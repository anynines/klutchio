...
type reconciler struct {
	scope bindv1alpha1.Scope

	listServiceExports func(ns string) ([]*bindv1alpha1.APIServiceExport, error)
...
func (r *reconciler) reconcile(ctx context.Context, clusterBinding *bindv1alpha1.ClusterBinding) error {
	var errs []error
...
func (r *reconciler) ensureClusterBindingConditions(ctx context.Context, clusterBinding *bindv1alpha1.ClusterBinding) error {
	if clusterBinding.Status.LastHeartbeatTime.IsZero() {
		conditions.MarkFalse(clusterBinding,
			bindv1alpha1.ClusterBindingConditionHealthy,
...
func (r *reconciler) ensureRBACClusterRole(ctx context.Context, clusterBinding *bindv1alpha1.ClusterBinding) error {
	name := "klutch-binder-" + clusterBinding.Namespace
...
func (r *reconciler) ensureRBACClusterRoleBinding(ctx context.Context, clusterBinding *bindv1alpha1.ClusterBinding) error {
	name := "klutch-binder-" + clusterBinding.Namespace
	...
	if r.scope != bindv1alpha1.ClusterScope {
...
func (r *reconciler) ensureRBACRoleBinding(ctx context.Context, clusterBinding *bindv1alpha1.ClusterBinding) error {
	binding, err := r.getRoleBinding(clusterBinding.Namespace, "klutch-binder")
	if err != nil && !errors.IsNotFound(err) {
		return fmt.Errorf("failed to get RoleBinding \"klutch-binder\": %w", err)
	}
