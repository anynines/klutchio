...
func (c *reconciler) reconcile(ctx context.Context, sns *bindv1alpha1.APIServiceNamespace) error {
		...
		ns = &corev1.Namespace{
			ObjectMeta: metav1.ObjectMeta{
				...
				Annotations: map[string]string{
					bindv1alpha1.APIServiceNamespaceAnnotationKey: sns.Namespace + "/" + sns.Name,
				},
			},
		}
	...
	if c.scope == bindv1alpha1.NamespacedScope {
...
func (c *reconciler) ensureRBACRoleBinding(ctx context.Context, ns string, sns *bindv1alpha1.APIServiceNamespace) error {
	objName := "klutch-binder"
	...
	expected := &rbacv1.RoleBinding{
		...
		RoleRef: rbacv1.RoleRef{
			Kind:     "ClusterRole",
			Name:     "klutch-binder-" + sns.Namespace,
