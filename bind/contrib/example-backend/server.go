...
func NewServer(config *Config) (*Server, error) {
	...
	s.ClusterBinding, err = clusterbinding.NewController(
		config.ClientConfig,
		bindv1alpha1.Scope(config.Options.ConsumerScope),
		config.BindInformers.KlutchBind().V1alpha1().ClusterBindings(),
		config.BindInformers.KlutchBind().V1alpha1().APIServiceExports(),
	...
	s.ServiceNamespace, err = servicenamespace.NewController(
		config.ClientConfig,
		bindv1alpha1.Scope(config.Options.ConsumerScope),
		config.BindInformers.KlutchBind().V1alpha1().APIServiceNamespaces(),
		config.BindInformers.KlutchBind().V1alpha1().ClusterBindings(),
		config.BindInformers.KlutchBind().V1alpha1().APIServiceExports(),
		config.KubeInformers.Core().V1().Namespaces(),
	...
	s.ServiceExportRequest, err = serviceexportrequest.NewController(
		config.ClientConfig,
		bindv1alpha1.Scope(config.Options.ConsumerScope),
		bindv1alpha1.Isolation(config.Options.ClusterScopedIsolation),
		config.BindInformers.KlutchBind().V1alpha1().APIServiceExportRequests(),
		config.BindInformers.KlutchBind().V1alpha1().APIServiceExports(),
		config.ApiextensionsInformers.Apiextensions().V1().CustomResourceDefinitions(),
	)
	if err != nil {
