...
func (h *handler) handleServiceExport(w http.ResponseWriter, r *http.Request) {
	...
	provider := &bindv1alpha1.BindingProvider{
		TypeMeta: metav1.TypeMeta{
			APIVersion: bindv1alpha1.GroupVersion,
			Kind:       "BindingProvider",
		},
		Version:            ver,
		ProviderPrettyName: "example-backend",
		AuthenticationMethods: []bindv1alpha1.AuthenticationMethod{
			{
				Method: "OAuth2CodeGrant",
				OAuth2CodeGrant: &bindv1alpha1.OAuth2CodeGrant{
...
func (h *handler) handleCallback(w http.ResponseWriter, r *http.Request) {
	...
	cookieName := "klutch-bind-" + authCode.SessionID
...
func (h *handler) handleResources(w http.ResponseWriter, r *http.Request) {
	...
	for _, crd := range crds {
		crd := crd
		if h.scope == bindv1alpha1.ClusterScope || crd.Spec.Scope == apiextensionsv1.NamespaceScoped {
...
func (h *handler) handleBind(w http.ResponseWriter, r *http.Request) {
	...
	cookieName := "klutch-bind-" + r.URL.Query().Get("s")
	...
	request := bindv1alpha1.APIServiceExportRequestResponse{
		TypeMeta: metav1.TypeMeta{
			APIVersion: bindv1alpha1.SchemeGroupVersion.String(),
			Kind:       "APIServiceExportRequest",
		},
		ObjectMeta: bindv1alpha1.NameObjectMeta{
		...
		Spec: bindv1alpha1.APIServiceExportRequestSpec{
			Resources: []bindv1alpha1.APIServiceExportRequestResource{
				{
					GroupResource:    bindv1alpha1.GroupResource{Group: group, Resource: resource},
	...
	response := bindv1alpha1.BindingResponse{
		TypeMeta: metav1.TypeMeta{
			APIVersion: bindv1alpha1.SchemeGroupVersion.String(),
			Kind:       "BindingResponse",
		},
		Authentication: bindv1alpha1.BindingResponseAuthentication{
			OAuth2CodeGrant: &bindv1alpha1.BindingResponseAuthenticationOAuth2CodeGrant{
