...
func NewOptions() *Options {
	...
	return &Options{
		...
		ExtraOptions: ExtraOptions{
			...
			ConsumerScope:          string(bindv1alpha1.NamespacedScope),
			ClusterScopedIsolation: string(bindv1alpha1.IsolationPrefixed),
		},
	}
}
...
func (options *Options) Complete() (*CompletedOptions, error) {
	...
	if strings.ToLower(options.ConsumerScope) == "namespaced" {
		options.ConsumerScope = string(bindv1alpha1.NamespacedScope)
	}
	if strings.ToLower(options.ConsumerScope) == "cluster" {
		options.ConsumerScope = string(bindv1alpha1.ClusterScope)
	}
	switch strings.ToLower(options.ClusterScopedIsolation) {
	case "prefixed":
		options.ClusterScopedIsolation = string(bindv1alpha1.IsolationPrefixed)
	case "namespaced":
		options.ClusterScopedIsolation = string(bindv1alpha1.IsolationNamespaced)
	case "none":
		options.ClusterScopedIsolation = string(bindv1alpha1.IsolationNone)
	}
