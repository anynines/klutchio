...
func (b *BindAPIServiceOptions) createAPIServiceBindings(ctx context.Context, config *rest.Config, request *bindv1alpha1.APIServiceExportRequest, secretName string) ([]*bindv1alpha1.APIServiceBinding, error) {
	...
	for _, resource := range request.Spec.Resources {
		...
		if err != nil && !apierrors.IsNotFound(err) {
			...
			if err != nil && !apierrors.IsNotFound(err) {
				...
				if !helpers.IsOwnedByBinding(existing.Name, existing.UID, crd.OwnerReferences) {
					return nil, fmt.Errorf("CustomResourceDefinition %s exists, but is not owned by klutch-bind", crd.Name)
				}
			}
			continue
		}

		var permissionClaims []bindv1alpha1.AcceptablePermissionClaim
		for _, c := range resource.PermissionClaims {
			...
			var state bindv1alpha1.AcceptablePermissionClaimState
			if accepted {
				state = bindv1alpha1.ClaimAccepted
			} else {
				state = bindv1alpha1.ClaimRejected
			}

			permissionClaims = append(permissionClaims, bindv1alpha1.AcceptablePermissionClaim{
				PermissionClaim: c,
				State:           state,
			})
		}
...
func printPermissionClaim(w io.Writer, p bindv1alpha1.PermissionClaim) error {
	var b bytes.Buffer
...
func writeFirstLines(b *bytes.Buffer, groupResource string, claim bindv1alpha1.PermissionClaim) error {
	var err error
...
func writeCreate(b io.StringWriter, claim bindv1alpha1.PermissionClaim) error {
	var err error
...
func writeOnConflict(b io.StringWriter, claim bindv1alpha1.PermissionClaim) error {
	var err error
...
func writeUpdateClause(b *bytes.Buffer, claim bindv1alpha1.PermissionClaim) error {
	var err error
...
func (opt BindAPIServiceOptions) promptYesNo(p bindv1alpha1.PermissionClaim) (bool, error) {
