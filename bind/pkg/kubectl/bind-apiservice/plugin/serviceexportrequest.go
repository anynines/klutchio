...
func (b *BindAPIServiceOptions) createServiceExportRequest(
...
) (*bindv1alpha1.APIServiceExportRequest, error) {
	...
	created, err := bindRemoteClient.KlutchBindV1alpha1().APIServiceExportRequests(ns).Create(ctx, request, metav1.CreateOptions{})
	...
	var result *bindv1alpha1.APIServiceExportRequest
	if err := wait.PollImmediate(1*time.Second, 10*time.Minute, func() (bool, error) {
		request, err := bindRemoteClient.KlutchBindV1alpha1().APIServiceExportRequests(ns).Get(ctx, created.Name, metav1.GetOptions{})
		if err != nil && !apierrors.IsNotFound(err) {
			return false, err
		} else if apierrors.IsNotFound(err) {
			return false, fmt.Errorf("APIServiceExportRequest %s was deleted by the service provider", created.Name)
		}
		if request.Status.Phase == bindv1alpha1.APIServiceExportRequestPhaseSucceeded {
			result = request
			return true, nil
		}
		if request.Status.Phase == bindv1alpha1.APIServiceExportRequestPhaseFailed {
...
func (b *BindAPIServiceOptions) printTable(ctx context.Context, config *rest.Config, bindings []*bindv1alpha1.APIServiceBinding) error {
	printer := printers.NewTablePrinter(printers.PrintOptions{
		WithKind: true,
		Kind:     bindv1alpha1.SchemeGroupVersion.WithKind("APIServiceBinding").GroupKind(),
	})
	...
	tableConfig.GroupVersion = &bindv1alpha1.SchemeGroupVersion
