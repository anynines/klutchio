...
func (b *BindOptions) Run(ctx context.Context, urlCh chan<- string) error {
	...
	if provider.APIVersion != bindv1alpha1.GroupVersion {
		return fmt.Errorf("unsupported binding provider version: %q", provider.APIVersion)
	}

	ns, err := kubeClient.CoreV1().Namespaces().Get(ctx, "klutch-bind", metav1.GetOptions{})
	if err != nil && !apierrors.IsNotFound(err) {
		...
		if ns, err = kubeClient.CoreV1().Namespaces().Create(ctx, ns, metav1.CreateOptions{}); err != nil {
			...
			fmt.Fprintf(b.Options.IOStreams.ErrOut, "📦 Created klutch-bind namespace.\n") // nolint: errcheck
		}
	}
	...
	// verify the response
	if gvk.GroupVersion() != bindv1alpha1.SchemeGroupVersion || gvk.Kind != "BindingResponse" {
		return fmt.Errorf("unexpected response type %s, only supporting %s", gvk, bindv1alpha1.SchemeGroupVersion.WithKind("BindingResponse"))
	}
	bindingResponse, ok := response.(*bindv1alpha1.BindingResponse)
	if !ok {
	...
	var apiRequests []*bindv1alpha1.APIServiceExportRequestResponse
	for i, request := range bindingResponse.Requests {
		...
		if got, expected := meta.APIVersion, bindv1alpha1.SchemeGroupVersion.String(); got != expected {
			return fmt.Errorf("unexpected response: request #%d is not %s, got %s", i, expected, got)
		}
		var apiRequest bindv1alpha1.APIServiceExportRequestResponse
