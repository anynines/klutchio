...
import (
	...
	"github.com/anynines/klutchio/bind/pkg/konnector/controllers/cluster/clusterbinding"
	"github.com/anynines/klutchio/bind/pkg/konnector/controllers/cluster/namespacedeletion"
	"github.com/anynines/klutchio/bind/pkg/konnector/controllers/cluster/servicebinding"
	"github.com/anynines/klutchio/bind/pkg/konnector/controllers/cluster/serviceexport"
	"github.com/anynines/klutchio/bind/pkg/konnector/controllers/dynamic"
)

const (
	controllerName = "klutch-bind-konnector-cluster"
...
func (c *controller) Start(ctx context.Context) {
		...
		case <-ctx.Done():
			...
			c.updateServiceBindings(ctx, func(binding *bindv1alpha1.APIServiceBinding) {
				conditions.MarkFalse(
					binding,
					bindv1alpha1.APIServiceBindingConditionInformersSynced,
					"InformerSyncTimeout",
	...
	c.updateServiceBindings(ctx, func(binding *bindv1alpha1.APIServiceBinding) {
		conditions.MarkTrue(binding, bindv1alpha1.APIServiceBindingConditionInformersSynced)
	})
...
func (c *controller) updateServiceBindings(ctx context.Context, update func(*bindv1alpha1.APIServiceBinding)) {
	...
	for _, obj := range objs {
		...
		if !reflect.DeepEqual(binding.Status.Conditions, orig.Status.Conditions) {
			...
			if _, err := c.bindClient.KlutchBindV1alpha1().APIServiceBindings(binding.Namespace).UpdateStatus(ctx, binding, metav1.UpdateOptions{}); err != nil {
