...
func TestEnsureCRDs(t *testing.T) {
	tests := []struct {
		name             string
		bindingName      string
		getServiceExport func(name string) (*bindv1alpha1.APIServiceExport, error)
		...
		},
		{
			...
			expectConditions: conditionsapi.Conditions{
				conditionsapi.Condition{
					...
					Message:  "CustomResourceDefinition foo is not owned by klutch.anynines.com.",
				},
			},
		},
	}
...
func newGetServiceExport(name string, crd *bindv1alpha1.APIServiceExport) func(name string) (*bindv1alpha1.APIServiceExport, error) {
	return func(n string) (*bindv1alpha1.APIServiceExport, error) {
		if n == name {
			return crd, nil
		}
		return nil, errors.NewNotFound(bindv1alpha1.SchemeGroupVersion.WithResource("apiserviceexports").GroupResource(), "not found")
	}
}

func newServiceExport(name string) *bindv1alpha1.APIServiceExport {
	return &bindv1alpha1.APIServiceExport{
		ObjectMeta: metav1.ObjectMeta{
			Name: name,
		},
...
func newBinding(name string) *bindv1alpha1.APIServiceBinding {
