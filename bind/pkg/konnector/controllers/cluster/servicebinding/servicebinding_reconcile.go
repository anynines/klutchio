...
type reconciler struct {
	...
	getServiceExport        func(ns string) (*bindv1alpha1.APIServiceExport, error)
...
func (r *reconciler) reconcile(ctx context.Context, binding *bindv1alpha1.APIServiceBinding) error {
	var errs []error
...
func (r *reconciler) ensureValidServiceExport(ctx context.Context, binding *bindv1alpha1.APIServiceBinding) error {
	if _, err := r.getServiceExport(binding.Name); err != nil && !errors.IsNotFound(err) {
		return err
	} else if errors.IsNotFound(err) {
		conditions.MarkFalse(
			binding,
			bindv1alpha1.APIServiceBindingConditionConnected,
...
func (r *reconciler) ensureCRDs(ctx context.Context, binding *bindv1alpha1.APIServiceBinding) error {
	var errs []error
	...
	if err != nil && !errors.IsNotFound(err) {
		...
		conditions.MarkTrue(binding, bindv1alpha1.APIServiceBindingConditionConnected)
	...
	if !kubebindhelpers.IsOwnedByBinding(binding.Name, binding.UID, existing.OwnerReferences) {
		conditions.MarkFalse(
			...
			conditionsapi.ConditionSeverityError,
			"CustomResourceDefinition %s is not owned by klutch.anynines.com.",
			binding.Name,
		)
		return nil
	}
...
func (r *reconciler) ensurePrettyName(ctx context.Context, binding *bindv1alpha1.APIServiceBinding) error {
