...
import (
	...
	"github.com/anynines/klutch/bind/pkg/konnector/adopt"
)
...
func (r *reconciler) reconcile(ctx context.Context, obj *unstructured.Unstructured) error {
	...
	if ns != "" {
		...
		if err != nil && !errors.IsNotFound(err) {
			...
			sn, err = r.createServiceNamespace(ctx, &bindv1alpha1.APIServiceNamespace{
...
func (r *reconciler) ensureDownstreamFinalizer(ctx context.Context, obj *unstructured.Unstructured) (*unstructured.Unstructured, error) {
	...
	for _, f := range obj.GetFinalizers() {
		if f == bindv1alpha1.DownstreamFinalizer {
			found = true
			break
		}
	}
	...
	if !found {
		...
		obj = obj.DeepCopy()
		obj.SetFinalizers(append(obj.GetFinalizers(), bindv1alpha1.DownstreamFinalizer))
		var err error
