...
func getOperationResponse() *v2.GetOperationResponse {
...
func TestGetOperation(t *testing.T) {
	cases := []struct {
		name     string
		reaction fake.GetOperationReactionInterface
		response *v2.GetOperationResponse
		err      error
	}{
		{
		...
		{
			name: "response",
			reaction: &fake.GetOperationReaction{
		...
		{
			name: "nil static reaction",
			reaction: func() fake.GetOperationReactionInterface {
				var nilStaticReaction *fake.GetOperationReaction
				return nilStaticReaction
			}(),
			err: fake.UnexpectedActionError(),
		},
	...
	for _, tc := range cases {
		fakeClient := &fake.FakeClient{
			GetOperationReaction: tc.reaction,
		}

		response, err := fakeClient.GetOperation(&v2.GetOperationRequest{})
		...
		if e, a := fake.GetOperation, actions[0].Type; e != a {
			t.Errorf("%v: unexpected action type; expected %v, got %v", tc.name, e, a)
		}
	}
}
...
func TestCheckAvailability(t *testing.T) {
	cases := []struct {
		name        string
		reaction    fake.CheckAvailabilityReactionInterface
		expectedErr error
	}{
		{
			name:        "unexpected action",
			expectedErr: fake.UnexpectedActionError(),
		},
		{
			name:     "react with no error",
			reaction: fake.CheckAvailabilityReaction(func() error { return nil }),
		},
		{
			name: "react with error",
			reaction: fake.CheckAvailabilityReaction(func() error {
	...
	for _, tc := range cases {
		...
		if e, a := fake.CheckAvailability, actions[0].Type; e != a {
			t.Errorf("%v: unexpected action type; expected %v, got %v", tc.name, e, a)
		}
	}
}
