...
// GetInstanceNotAllowedError is an error type signifying that doing a GET to
// fetch a service instance is not allowed for this client.
type GetServiceInstanceNotAllowedError struct {
	reason string
}

func (e GetServiceInstanceNotAllowedError) Error() string {
	return fmt.Sprintf(
		"GetServiceInstance not allowed: %s",
		e.reason,
	)
}
...
// AvailabilityInvalidStatusError indicates that the availability check received an unexpected
// HTTP status.
type AvailabilityInvalidStatusError struct {
	StatusCode int
}

func (e AvailabilityInvalidStatusError) Error() string {
	return fmt.Sprintf("Received unexpected status %d", e.StatusCode)
}
...
func (e OperationStateError) Error() string {
	return fmt.Sprintf("Operation is in failed state %q", e.State)
}
