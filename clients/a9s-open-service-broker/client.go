...
// cachedCatalog contains the fields to store the key and value for caching purposes
...
func NewClient(config *ClientConfiguration) (Client, error) {
	...
	// if no value is set then we set the cache refresh timer to 15 seconds
	if config.CacheFreshnessSeconds == nil {
		config.CacheFreshnessSeconds = pointer.Int(15)
	}

	if *config.CacheFreshnessSeconds < 0 {
		return nil, fmt.Errorf(errCacheFreshnessNegative)
	}
	...
	// if the value of CacheFreshnessSeconds is explicitly set to 0 then the cache gets disabled.
	if *config.CacheFreshnessSeconds == 0 {
		c.cache = nil
	} else {
		c.cache = cache.NewTTLStore(cacheKeyFunc, time.Duration(*config.CacheFreshnessSeconds)*time.Second)
	}
...
func cacheKeyFunc(obj interface{}) (string, error) {
	input, ok := obj.(cachedCatalog)
	if !ok {
		return "", fmt.Errorf("cannot use cache with objects that are not of type keyValue")
	} else {
