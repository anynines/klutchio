...
ensure_remote_git_branch() {
    if ! git remote | grep -q "origin"; then
        log_error "No git remotes origin found. Please add a git remote with that name and retry."
        exit 1
    fi
...
build_docker_images() {
    ...
    KO_DOCKER_REPO="${ECR_REGISTRY_ADDRESS}/klutch/konnector" ko build "$KLUTCHIO_REPO/bind/cmd/konnector" --bare -t "${VERSION_NUMBER}" --platform=linux/amd64,linux/arm64

    log_info "Building example-backend image..."
    KO_DOCKER_REPO="${ECR_REGISTRY_ADDRESS}/klutch/example-backend" ko build "$KLUTCHIO_REPO/cmd/example-backend" --bare -t "${VERSION_NUMBER}" --platform=linux/amd64,linux/arm64
